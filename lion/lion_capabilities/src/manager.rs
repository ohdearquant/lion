...
impl CapabilityManagerImpl {
    ...
    /// Get capabilities granted to a plugin
    pub fn get_granted_capabilities(&self, plugin_id: PluginId) -> Vec<CoreCapability> {
        let capabilities = self.plugin_capabilities
            .get(&plugin_id)
            .map(|caps| {
                caps.iter()
