    ...
    fn check_limits(&self, usage: &ResourceUsage) -> Result<()> {
        ...
        if let Some(limit) = self.memory_limit {
            if usage.memory_bytes > limit {
                return Err(IsolationError::ResourceLimitExceeded(
                    format!("Memory usage ({} bytes) exceeds limit ({} bytes)", usage.memory_bytes, limit)
                ).into());
            }
        }
        ...
        if let Some(limit) = self.cpu_time_limit {
            if usage.cpu_time_us > limit {
                return Err(IsolationError::ResourceLimitExceeded(
                    format!("CPU time ({} us) exceeds limit ({} us)", usage.cpu_time_us, limit)
                ).into());
            }
        }
        ...
        if let Some(limit) = self.function_timeout {
            ...
            if elapsed_ms > limit {
                return Err(IsolationError::ResourceLimitExceeded(
                    format!("Function timeout ({} ms) exceeded", limit)
                ).into());
            }
        }
...
mod tests {
    ...
    fn test_check_limits() {
        let limiter = DefaultResourceLimiter::new(
            Some(100),
            Some(100),
            Some(100),
        );
