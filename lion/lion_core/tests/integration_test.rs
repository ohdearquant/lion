...
use lion_core::types::{
    AccessRequest, ErrorPolicy, ExecutionOptions, MemoryRegion, MemoryRegionType,
    NodeType, PluginConfig, PluginMetadata, PluginState, ResourceUsage, Workflow, WorkflowNode,
};
...
impl Capability for FileCapability {
            ...
            AccessRequest::File { path, read, write, execute } => {
                ...
                if !path.starts_with(&self.path) {
                    return Err(CapabilityError::PermissionDenied(
                        format!("Path {} is not within allowed path {}", path.display(), self.path.display())
                    ));
                }
                ...
                if *write && !self.write {
                    return Err(CapabilityError::PermissionDenied("Write not allowed".into()));
                }
                
                if *execute && !self.execute {
                    return Err(CapabilityError::PermissionDenied("Execute not allowed".into()));
                }
    ...
    fn constrain(&self, constraints: &[lion_core::traits::Constraint]) -> Result<Box<dyn Capability>, CapabilityError> {
                ...
                Constraint::FilePath(path) => {
                    ...
                    if !new_path.starts_with(&self.path) {
                        return Err(CapabilityError::ConstraintError(
                            format!("New path {} is not within original path {}", 
                                    new_path.display(), self.path.display())
                        ));
                    }
                    new_cap.path = new_path;
                },
                Constraint::FileOperation { read, write, execute } => {
        ...
        match request {
            AccessRequest::Network { host, port, connect, listen } => {
                ...
                if *port < self.port_range.0 || *port > self.port_range.1 {
                    return Err(CapabilityError::PermissionDenied(
                        format!("Port {} is not in allowed range {}-{}", 
                ...
                if *connect && !self.connect {
                    return Err(CapabilityError::PermissionDenied("Connect not allowed".into()));
                }
                
                if *listen && !self.listen {
                    return Err(CapabilityError::PermissionDenied("Listen not allowed".into()));
                }
...
fn test_capability_integration() {
    ...
    let result = with_capability!(read_cap, &read_request, {
        Ok(42)
    });
    assert_eq!(result.unwrap(), 42);
    
    let result = with_capability!(read_cap, &write_request, {
        Ok(42)
    });
    assert!(result.is_err());
    ...
    let constraints = [
        ...
        Constraint::FileOperation { read: true, write: false, execute: false },
    ];
    
    let constrained_cap = tmp_cap.constrain(&constraints).unwrap();
    
    // Should allow read access to /tmp/specific/file.txt
    assert!(constrained_cap.permits(&AccessRequest::file_read("/tmp/specific/file.txt")).is_ok());
    
    // Should deny write access to /tmp/specific/file.txt
    assert!(constrained_cap.permits(&AccessRequest::file_write("/tmp/specific/file.txt")).is_err());
    
    // Should deny access to /tmp/other/file.txt
    assert!(constrained_cap.permits(&AccessRequest::file_read("/tmp/other/file.txt")).is_err());
}
...
fn test_complete_scenario() {
    ...
    let mut config = Config::new_map();
    config.set("plugins.data_processor.max_memory_mb", 100).unwrap();
    config.set("plugins.data_processor.timeout_ms", 5000).unwrap();
    
    let plugin_config = PluginConfig {
        max_memory_bytes: Some(config.get_as::<i64>("plugins.data_processor.max_memory_mb").unwrap() as usize * 1024 * 1024),
        function_timeout_ms: config.get_as::<i64>("plugins.data_processor.timeout_ms").map(|v| v as u64),
    ...
    let mut process_node = WorkflowNode::new_plugin_call(
        "Process Data", 
        plugin_id.to_string(), 
        "process_csv"
    );
    ...
    let mut visualize_node = WorkflowNode::new_plugin_call("Visualize", "visualizer", "create_chart");
