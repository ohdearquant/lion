...
mod tests {
    ...
    impl Capability for TestCapability {
        ...
        fn permits(&self, _request: &AccessRequest) -> std::result::Result<(), CapabilityError> {
            if self.permitted {
                Ok(())
            } else {
                Err(CapabilityError::PermissionDenied("Test denial".into()))
            }
        }
    }
    ...
    fn test_with_capability_macro() {
        ...
        let result = with_capability!(permitted_cap, &request, {
            Ok(42)
        });
        assert_eq!(result.unwrap(), 42);
        
        // Should not execute block and return error
        let result = with_capability!(denied_cap, &request, {
            Ok(42)
        });
        assert!(result.is_err());
    }
    ...
    fn test_wrap_err_macro() {
        ...
        let result = (|| -> Result<i32> {
            let val = wrap_err!(ok_result, 
                              CapabilityError::NotGranted("Test".into()));
            Ok(val)
        })();
        assert_eq!(result.unwrap(), 42);
        
        let err_result: std::result::Result<i32, std::io::Error> = 
            Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Test IO error"));
        
        // Should wrap the error
        let result = (|| -> Result<i32> {
            let val = wrap_err!(err_result, 
                              CapabilityError::NotGranted("Test".into()));
            Ok(val)
        })();
        assert!(result.is_err());
