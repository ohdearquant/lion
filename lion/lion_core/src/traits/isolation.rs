...
use crate::types::{PluginConfig, PluginState, ResourceUsage, MemoryRegion};
...
pub trait IsolationBackend: Send + Sync {
    ...
    fn share_memory_region(&self, plugin_id: &PluginId, region_id: &RegionId, read_only: bool) -> Result<()> {
    ...
    fn set_resource_limit(&self, id: &PluginId, limit_type: ResourceLimitType, value: u64) -> Result<()> {
        Err(IsolationError::PluginNotLoaded(*id).into())
    }
...
mod tests {
    ...
    impl IsolationBackend for TestIsolationBackend {
        fn load_plugin(&self, id: PluginId, module_bytes: &[u8], _config: &PluginConfig) -> Result<()> {
            self.plugins.lock().unwrap().insert(id, module_bytes.to_vec());
        ...
        fn create_memory_region(&self, size: usize) -> Result<MemoryRegion> {
            ...
            self.memory_regions.lock().unwrap().insert(region_id, vec![0; size]);
    ...
    fn test_load_unload_plugin() {
        ...
        // Load the plugin
        backend.load_plugin(plugin_id, &module_bytes, &config).unwrap();
    ...
    fn test_call_function() {
        ...
        let result = backend.call_function(&plugin_id, "test_function", &params).unwrap();
