...
impl CapabilityResolver {
    ...
    pub async fn can_access(
        ...
        match self.manager.check_permission(subject, object, operation).await {
            Ok(_) => {
                debug!("Access granted for '{}' to perform '{}' on '{}'", 
                    subject, operation, object);
                Ok(true)
            }
            Err(e) => {
                debug!("Access denied for '{}' to perform '{}' on '{}': {}", 
    ...
    pub async fn authorize(
        ...
        // Use the capability manager to check permission
        self.manager.check_permission(subject, object, operation).await
            .context(format!("Authorization failed for '{}' to perform '{}' on '{}'",
                subject, operation, object))
    }
...
mod tests {
    ...
    async fn test_can_access() {
        ...
        // Check access
        assert!(resolver.can_access("subject1", "object1", "read").await.unwrap());
        assert!(resolver.can_access("subject1", "object1", "write").await.unwrap());
        assert!(!resolver.can_access("subject1", "object1", "execute").await.unwrap());
        assert!(!resolver.can_access("subject2", "object1", "read").await.unwrap());
    }
    ...
    async fn test_authorize() {
        ...
        // Test authorize
        resolver.authorize("subject1", "object1", "read").await.unwrap();
        resolver.authorize("subject1", "object1", "write").await.unwrap();
        
        // These should fail
        assert!(resolver.authorize("subject1", "object1", "execute").await.is_err());
        assert!(resolver.authorize("subject2", "object1", "read").await.is_err());
    }
    ...
    async fn test_batch_authorize() {
        ...
        resolver.batch_authorize("subject1", "object1", &["read", "write"]).await.unwrap();
        
        // This should fail because execute is not granted
        assert!(resolver.batch_authorize("subject1", "object1", &["read", "execute"]).await.is_err());
    }
}