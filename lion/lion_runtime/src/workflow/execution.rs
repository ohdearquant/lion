...
impl WorkflowExecutor {
    ...
    pub async fn start_workflow(
        ...
        self.tx.send(ExecutorMessage::Start(workflow_id, input)).await
    ...
    pub async fn pause_workflow(&self, workflow_id: WorkflowId) -> Result<()> {
        ...
        self.tx.send(ExecutorMessage::Pause(workflow_id)).await
    ...
    pub async fn resume_workflow(&self, workflow_id: WorkflowId) -> Result<()> {
        ...
        self.tx.send(ExecutorMessage::Resume(workflow_id)).await
    ...
    pub async fn cancel_workflow(&self, workflow_id: WorkflowId) -> Result<()> {
        ...
        self.tx.send(ExecutorMessage::Cancel(workflow_id)).await
        ...
        while let Some(msg) = rx.recv().await {
            match msg {
                ...
                ExecutorMessage::NodeCompleted(workflow_id, node_id, output) => {
                    if let Err(e) = self.handle_node_completed(workflow_id, node_id, output).await {
                        error!("Failed to handle node completion: {}", e);
                    }
                }
                ExecutorMessage::NodeFailed(workflow_id, node_id, error_msg) => {
                    if let Err(e) = self.handle_node_failed(workflow_id, node_id, error_msg).await {
    ...
    async fn handle_start(&self, workflow_id: WorkflowId, input: serde_json::Value) -> Result<()> {
        ...
        let mut states = self.workflow_states.write().await;
        let state = states.get_mut(&workflow_id)
            .ok_or_else(|| ExecutionError::WorkflowNotRunning(workflow_id.clone()))?;
        ...
        let ready_nodes: Vec<NodeId> = state.in_degree
            .iter()
        ...
        for node_id in ready_nodes {
            self.execute_node(workflow_id.clone(), node_id, input.clone()).await?;
        }
    ...
    async fn handle_resume(&self, workflow_id: WorkflowId) -> Result<()> {
        ...
        if state.status == WorkflowStatus::Paused {
            ...
            let ready_nodes: Vec<(NodeId, Vec<serde_json::Value>)> = state.definition.nodes
                .iter()
                .filter(|node| {
                ...
                .map(|node| {
                    ...
                    let dep_outputs: Vec<serde_json::Value> = node.dependencies
                        .iter()
            ...
            for (node_id, dep_outputs) in ready_nodes {
                ...
                self.execute_node(workflow_id.clone(), node_id, input).await?;
            }
        }
    ...
    async fn handle_node_completed(
        ...
        debug!("Node completed: {:?} in workflow {:?}", node_id, workflow_id);
        ...
        let dependent_nodes: Vec<(NodeId, bool)> = state.definition.nodes
            .iter()
            .filter(|node| node.dependencies.contains(&node_id))
            .map(|node| {
                // Decrease in-degree
                let new_in_degree = state.in_degree.get_mut(&node.id).map(|count| {
                    *count -= 1;
                    *count
                }).unwrap_or(0);
        ...
        let all_completed = state.node_statuses
            .values()
        ...
        let ready_nodes: Vec<(NodeId, Vec<serde_json::Value>)> = dependent_nodes
            ...
            .map(|(node_id, _)| {
                // Get the node
                let node = state.definition.nodes
                    .iter()
                    .find(|n| n.id == node_id)
                    .unwrap();
    ...
    async fn handle_node_failed(
        ...
        error!("Node failed: {:?} in workflow {:?}: {}", node_id, workflow_id, error_msg);
        ...
        tokio::spawn(async move {
            let result = match node.plugin_id {
                Some(plugin_id) => {
                    ...
                    match plugin_manager.call_plugin_function(
                        &plugin_id,
                        &node.function,
                        input,
                    ).await {
                        ...
                        Err(e) => {
                            // Node execution failed
                            if let Err(e) = tx.send(
                                ExecutorMessage::NodeFailed(workflow_id, node_id, e.to_string())
                            ).await {
                ...
                None => {
                    ...
                    if let Err(e) = tx.send(
                        ExecutorMessage::NodeCompleted(workflow_id, node_id, input)
                    ).await {
                        error!("Failed to send node completion message: {}", e);
                    }
                }
