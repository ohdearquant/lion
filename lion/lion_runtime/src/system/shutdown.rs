...
impl ShutdownHandle {
    ...
    pub async fn wait_for_shutdown(&mut self) -> Result<()> {
        self.receiver.recv().await.map_err(|e| anyhow::anyhow!("Shutdown signal error: {}", e))
    }
...
impl ShutdownManager {
    ...
    pub async fn request_shutdown(&self) -> Result<()> {
        ...
        *in_progress = true;
        ...
        self.shutdown_tx.send(())
            .map_err(|_| ShutdownError::ComponentFailed("Failed to send shutdown signal".to_string()))?;
    ...
    async fn wait_for_completion(&self, count: usize) -> Result<()> {
        ...
        match self.completion.acquire_many(count as u32).await {
            ...
            Err(e) => Err(ShutdownError::ComponentFailed(
    ...
    fn log_incomplete_components(&self) {
        ...
        error!("{} of {} components did not complete shutdown", incomplete_count, total_count);
