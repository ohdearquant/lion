...
mod tests {
    ...
    async fn test_discover_plugins() {
        ...
        fs::write(temp_path.join("plugin1.wasm"), b"test plugin 1")
            .await
            .unwrap();
        fs::write(temp_path.join("plugin2.wasm"), b"test plugin 2")
            .await
            .unwrap();
        fs::write(temp_path.join("not_a_plugin.txt"), b"not a plugin")
            .await
            .unwrap();
