...
impl PluginManager {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        self.load_plugins_from_directory(&self.config.plugin_directory).await?;
    ...
    pub async fn load_plugins_from_directory(&self, directory: &str) -> Result<()> {
        ...
        for plugin_metadata in discovered_plugins {
            ...
            // Register the plugin
            self.registry.register_plugin(plugin_metadata.clone()).await?;
            
            // Create the plugin lifecycle
            let isolation_manager = Arc::new(LifecycleManager::new(&plugin_metadata.path)?);
            let lifecycle = PluginLifecycle::new(plugin_metadata.clone(), isolation_manager).await?;
            
            // Load the plugin
            self.plugins.write().await.insert(plugin_metadata.id.clone(), Arc::new(lifecycle));
        }
        
        info!("Loaded {} plugins from directory", self.plugins.read().await.len());
    ...
    pub async fn register_plugin(&self, metadata: PluginMetadata, path: &str) -> Result<PluginId> {
        ...
        // Store the plugin
        self.plugins.write().await.insert(metadata.id.clone(), Arc::new(lifecycle));
    ...
    pub async fn initialize_plugin(&self, plugin_id: &PluginId, config: serde_json::Value) -> Result<()> {
    ...
    pub async fn start_plugin(&self, plugin_id: &PluginId) -> Result<()> {
        ...
        if state == PluginState::Loaded {
            ...
            let config = self.config.plugins.get(&metadata.name)
    ...
    pub async fn remove_plugin(&self, plugin_id: &PluginId) -> Result<()> {
        ...
        if let Ok(plugin) = self.plugins.read().await.get(plugin_id).ok_or_else(|| PluginManagerError::NotFound(plugin_id.clone())) {
            let state = plugin.get_state().await;
    ...
    pub async fn call_plugin_function(
        ...
        debug!("Calling function '{}' in plugin '{:?}'", function_name, plugin_id);
    ...
    pub async fn grant_capability(
        ...
        info!("Granting capability for object '{}' to plugin '{:?}'", object, plugin_id);
        ...
        let cap_id = self.capability_manager
...
mod tests {
    ...
    async fn test_manager_lifecycle() {
        ...
        // Register the plugin
        manager.register_plugin(metadata, &format!("{}/test-plugin", temp_path)).await.unwrap();
