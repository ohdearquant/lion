use crate::model::{WorkflowDefinition, WorkflowId, NodeId, NodeStatus, Edge, EdgeId};
use crate::state::checkpoint::{CheckpointManager, CheckpointError};
...
impl WorkflowState {
    ...
    fn check_workflow_completion(&mut self) {
        ...
        let all_completed = self.node_status.values().all(|status| {
            matches!(status, NodeStatus::Completed | NodeStatus::Failed | NodeStatus::Skipped)
        });
...
impl<S: StorageBackend> StateMachineManager<S> {
    ...
    pub async fn checkpoint_instance(&self, instance_id: &str) -> Result<String, StateMachineError> {
        // Get the instance
        let state_lock = match self.get_instance(instance_id).await {
            Some(state) => state,
            None => return Err(StateMachineError::Other(format!("Instance not found: {}", instance_id))),
        };
        ...
        if let Some(manager) = &self.checkpoint_manager {
            ...
            Err(StateMachineError::Other("Checkpoint manager not configured".to_string()))
        }
    }
    
    /// Get nodes that are ready to execute
    pub async fn get_ready_nodes(&self, instance_id: &str) -> Result<Vec<NodeId>, StateMachineError> {
        let state_lock = match self.get_instance(instance_id).await {
            Some(state) => state,
            None => return Err(StateMachineError::Other(format!("Instance not found: {}", instance_id))),
        };
    ...
    pub async fn set_node_running(&self, instance_id: &str, node_id: &NodeId) -> Result<(), StateMachineError> {
        let state_lock = match self.get_instance(instance_id).await {
            Some(state) => state,
            None => return Err(StateMachineError::Other(format!("Instance not found: {}", instance_id))),
        };
...
mod tests {
    ...
    async fn test_state_machine_basic_flow() {
        ...
        let node_ids: Vec<NodeId> = workflow.nodes.keys().cloned().collect();
        ...
        assert_eq!(ready_nodes[0], node_ids[0]); // First node should be ready
        
        // Mark start node as running
        manager.set_node_running(&instance_id, &node_ids[0]).await.unwrap();
        
        // Mark start node as completed
        let newly_ready = manager
            .set_node_completed(&instance_id, &node_ids[0], serde_json::json!({}))
            .await
            .unwrap();
        
        // Middle node should now be ready
        assert_eq!(newly_ready.len(), 1);
        assert_eq!(newly_ready[0], node_ids[1]);
        
        // Mark middle node as running
        manager.set_node_running(&instance_id, &node_ids[1]).await.unwrap();
        
        // Mark middle node as completed
        let newly_ready = manager
            .set_node_completed(&instance_id, &node_ids[1], serde_json::json!({}))
            .await
            .unwrap();
        
        // End node should now be ready
        assert_eq!(newly_ready.len(), 1);
        assert_eq!(newly_ready[0], node_ids[2]);
        
        // Mark end node as running
        manager.set_node_running(&instance_id, &node_ids[2]).await.unwrap();
        
        // Mark end node as completed
        let newly_ready = manager
            .set_node_completed(&instance_id, &node_ids[2], serde_json::json!({}))
            .await
            .unwrap();
    ...
    async fn test_state_machine_node_failure() {
        ...
        manager
            .set_node_failed(&instance_id, &node_ids[0], serde_json::json!({"error": "Test error"}))
            .await
            .unwrap();
        ...
        let state = instance.read().await;
        assert!(state.has_failed);
        assert_eq!(state.get_node_status(&node_ids[0]), Some(NodeStatus::Failed));
    }
