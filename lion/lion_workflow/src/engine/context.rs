use crate::model::{WorkflowDefinition, NodeId, EdgeId, NodeStatus};
use crate::state::{WorkflowState, ConditionResult};
...
impl ExecutionContext {
    ...
    pub fn get_current_node(&self) -> Result<&crate::model::Node, ContextError> {
        ...
        self.definition.get_node(&node_id)
    ...
    pub fn get_inputs(&self) -> Result<HashMap<NodeId, serde_json::Value>, ContextError> {
        ...
        // Get parent nodes
        let parent_nodes = self.definition.get_parent_nodes(&node_id)
    ...
    pub fn has_capability(&self, capability_id: &CapabilityId) -> Result<bool, ContextError> {
        ...
        checker.check_permission(subject, &object, action)
    ...
    pub fn evaluate_condition(&self, edge_id: &EdgeId) -> Result<ConditionResult, ContextError> {
        let edge = self.definition.get_edge(edge_id)
            .ok_or_else(|| ContextError::EdgeNotFound(*edge_id))?;
        
        // Get source node result
        let source_result = self.state.node_results.get(&edge.source)
            .ok_or_else(|| ContextError::InvalidState(format!("No result for source node {}", edge.source)))?;
...
fn evaluate_json_path(value: &serde_json::Value, path: &str) -> Result<bool, String> {
    ...
    for segment in segments {
        if segment.starts_with('[') && segment.ends_with(']') {
            // Array index
            let index = segment[1..segment.len()-1].parse::<usize>()
