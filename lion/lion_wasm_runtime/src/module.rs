...
impl WasmModule {
    ...
    pub fn from_file(engine: &Engine, path: &Path) -> Result<Arc<Module>, WasmRuntimeError> {
        let bytes = std::fs::read(path)
            .map_err(|e| WasmRuntimeError::Io(format!("Failed to read file: {}", e)))?;
        
        Self::from_binary(engine, &bytes)
    }
    
    /// Load and compile a WebAssembly module from binary data
    pub fn from_binary(engine: &Engine, bytes: &[u8]) -> Result<Arc<Module>, WasmRuntimeError> {
        // Try to compile as binary WebAssembly
        let module = Module::new(engine, bytes)
            .or_else(|_| {
                // If that failed, try to parse as WAT (text format)
                let binary = wat::parse_bytes(bytes)
                    .map_err(|e| WasmRuntimeError::InvalidWebAssembly(e.to_string()))?;
                Module::new(engine, &binary)
                    .map_err(|e| WasmRuntimeError::CompilationFailed(e.to_string()))
            })
            .map_err(|e| WasmRuntimeError::CompilationFailed(e.to_string()))?;
    ...
    pub fn from_url(engine: &Engine, url: &str) -> Result<Arc<Module>, WasmRuntimeError> {
        // Simple synchronous HTTP GET for the MVP
        // In a production system, this should be asynchronous
        let response = ureq::get(url)
            .call()
            .map_err(|e| WasmRuntimeError::Io(format!("Failed to fetch from URL: {}", e)))?;
        
        let bytes = response
            .into_reader()
            .bytes()
            .collect::<Result<Vec<u8>, _>>()
            .map_err(|e| WasmRuntimeError::Io(format!("Failed to read response body: {}", e)))?;
