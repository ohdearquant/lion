    ...
    fn to_core_manifest(
        ...
        let capabilities = toml
            ...
            .map(|cap| match cap.as_str() {
                ...
                _ if cap.starts_with("fs_read:") => {
                    let path = cap.strip_prefix("fs_read:").unwrap().to_string();
                    Ok(CoreCapability::FileSystemRead { path: Some(path) })
                }
                _ if cap.starts_with("fs_write:") => {
                    let path = cap.strip_prefix("fs_write:").unwrap().to_string();
                    Ok(CoreCapability::FileSystemWrite { path: Some(path) })
                }
                _ if cap.starts_with("network:") => {
                    let hosts = cap
                        ...
                        .collect();
                    Ok(CoreCapability::NetworkClient { hosts: Some(hosts) })
                }
