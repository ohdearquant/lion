...
pub async fn load_plugin(
    ...
    let engine = state.wasm_engine.as_ref()
    ...
    let export_names: Vec<_> = instance.exports(&mut store)
...
pub async fn invoke_plugin_function(
    ...
    debug!("Invoking function '{}' on plugin {}", function_name, plugin_id);
    
    // Get the plugin instance
    let plugins_wasm = state.plugins_wasm.read().await;
    let plugin = plugins_wasm.get(&plugin_id)
    ...
    let log_entry = LogEntry::new(
        LogLevel::Info,
        format!("Called function '{}' on plugin {} with result: {}", function_name, plugin_id, result),
        "wasm-engine",
    ).with_plugin_id(plugin_id);
...
pub async fn get_plugin_info(
    ...
    let plugins_wasm = state.plugins_wasm.read().await;
    let wasm_plugin = plugins_wasm.get(&plugin_id)
        .ok_or_else(|| format!("Plugin with ID {} not found", plugin_id))?;
